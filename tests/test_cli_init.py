...
from botfarm.bugtracker.linear.client import (
    LINEAR_API_URL,
    LinearAPIError,
    LinearClient,
)
...
class TestGenerateConfigYaml:
    ...
    def test_no_projects_section(self):
        ...
        # projects section should not be generated by init
        assert "\nprojects:" not in content
        assert "tracker_project" not in content
...
class TestInteractiveInit:
    ...
    def test_interactive_single_team(self, mock_cls, tmp_path, monkeypatch):
        ...
        runner = CliRunner()
        # Input: API key (team auto-selected since there's only one)
