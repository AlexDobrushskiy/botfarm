...
class StageTemplate:
    ...
    thinking_mode: str | None = None  # "enabled"/"adaptive"/"disabled", None = inherit user default
...
def duplicate_pipeline(
    ...
    for s in stage_rows:
        conn.execute(
            ...
            "model, effort, context_window, thinking_mode) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                ...
                s["context_window"], s["thinking_mode"],
            ),
        )
