...
def cleanup_qa_environment(
    ...
    *,
    ...
    if project_cfg.run_port:
        _kill_port_holder(project_cfg.run_port, label=label)
...
def _kill_port_holder(port: int, *, label: str) -> None:
    """Kill any process bound to *port* via ``fuser``."""
    try:
    ...
    try:
        ...
        for line in pids_str.splitlines():
            ...
            try:
                ...
                continue
            if str(proc_cwd).startswith(str(cwd_path)):
                try:
                    os.kill(pid, signal.SIGKILL)
